...
export default defineConfig(({ mode }: ConfigEnv): UserConfig => {
  const viteEnv = loadEnv(mode, path.resolve(__dirname, "./env"), [
    "VITE_",
    "APP_",
  ]);
  ...
  return {
    ...
    plugins: [
      ...
      Components({
        ...
        include: [/\.[tj]sx?$/, /\.vue$/, /\.vue\?vue/, /\.md$/], // /\.(s?c|le)ss$/,
      }),
      legacy({
        // 需要兼容的目标列表
        targets: [
      ...
      createSvgIconsPlugin({
        // 指定需要缓存的图标文件夹
        iconDirs: [
          fileURLToPath(new URL("./src/assets/images/icons", import.meta.url)),
        ],
        // 指定symbolId格式
